...
class MinPairHeap {
  ...
  compare(
    a: { sum: number; node: SolutionNode },
    b: { sum: number; node: SolutionNode },
  ) {
    if (a.sum !== b.sum) {
      return a.sum < b.sum;
    }
  ...
  siftDown() {
    ...
    while (true) {
      ...
      if (
        leftIdx < length &&
        this.compare(this.heap[leftIdx], this.heap[swapIdx])
      ) {
        swapIdx = leftIdx;
      }
      if (
        rightIdx < length &&
        this.compare(this.heap[rightIdx], this.heap[swapIdx])
      ) {
        swapIdx = rightIdx;
      }
      ...
      [this.heap[nodeIdx], this.heap[swapIdx]] = [
        this.heap[swapIdx],
        this.heap[nodeIdx],
      ];
      nodeIdx = swapIdx;
...
function minimumPairRemoval(nums: number[]): number {
  ...
  while (badPairs > 0) {
    ...
    if (!nodeVersion.has(left) || entry.version !== nodeVersion.get(left))
      continue;

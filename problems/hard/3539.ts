      ...
      for (let c = 0; c <= j; c++) {
        ...
        for (let l = 0; l <= k; l++) {
          ...
          for (let count_i = 0; j + count_i <= m; count_i++) {
            ...
            const val_to_add = Number(
              (((dp_val * comb) % BIG_MOD) * prod_term) % BIG_MOD,
            );

            // Update the next DP state.
            next_dp[new_j][new_c][new_l] =
              (next_dp[new_j][new_c][new_l] + val_to_add) % MOD;
          }
        }
      }
    }

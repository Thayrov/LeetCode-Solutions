...
function stoneGameVIII(stones: number[]): number {
  // Replace each value with its inclusive prefix sum.
  for (let i = 1; i < stones.length; ++i) {
    // Extend the previous prefix by the current stone.
    stones[i] += stones[i - 1];
  }

  // Seed the best difference with the forced full-prefix move.
  let best = stones[stones.length - 1];

  // Add each smaller legal prefix to the suffix of available choices.
  for (let i = stones.length - 2; i > 0; --i) {
    // Keep the stronger move between a later prefix and this prefix against optimal reply.
    best = Math.max(best, stones[i] - best);
  }

  // Return Alice's optimal score difference from the initial state.
  return best;
}

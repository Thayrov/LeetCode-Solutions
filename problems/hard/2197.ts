...
function replaceNonCoprimes(nums: number[]): number[] {
  ...
  // Stack will maintain the progressively reduced array state
  const stack: number[] = [];
  ...
  for (let num of nums) {
    ...
    // Push the merged value or original number into stack
    stack.push(curr);
  }

  // The final stack is the fully reduced array
  return stack;
}

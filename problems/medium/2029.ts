...
// Determine whether Alice can force a win from the stones' remainder counts.
function stoneGameIX(stones: number[]): boolean {
  // Count stones in the only three classes relevant to divisibility by three.
  const count = [0, 0, 0];

  // Visit each stone once to discard its irrelevant quotient.
  for (const stone of stones) {
    // Add the stone to its remainder class.
    ++count[stone % 3];
  }

  // Apply the optimal-play criterion selected by the parity of neutral stones.
  return (count[0] & 1) === 0
    ? count[1] > 0 && count[2] > 0

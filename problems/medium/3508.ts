...
class Router {
  ...
  private add(root: Node | null, key: number, delta: number): Node | null {
    ...
    if (M) {
      ...
      // create if inserting new timestamp
      M = {
        k: key,
        c: delta,
        s: delta,
        p: (Math.random() * 0xffffffff) >>> 0,
        l: null,
        r: null,
      };
    }

...
function matchPlayersAndTrainers(players: number[], trainers: number[]): number {
  ...
  // Continue while there are unprocessed players and trainers
  while (i < players.length && j < trainers.length) {
    // Check if current player can be matched with current trainer
    if (players[i] <= trainers[j]) {
      // Match found: increment match counter and move to next player
      matches++;
      i++;
    }
    // Always move to next trainer (either after successful match or skip unsuitable one)
    j++;
  }

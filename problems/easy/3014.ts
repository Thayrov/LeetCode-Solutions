...
// Compute the minimum pushes for the distinct letters in the word.
function minimumPushes(word: string): number {
  // Count the complete eight-letter cost layers assigned across the available keys.
  const fullLayers = Math.floor(word.length / 8);
  // Sum complete layers and charge every remaining letter at the next layer's push cost.
  return 4 * fullLayers * (fullLayers + 1) + (word.length % 8) * (fullLayers + 1);
}

...
function applyOperations(nums: number[]): number[] {
  // First pass: Apply multiplication operations
  ...
  // Keep track of where we should place the next non-zero element
  let nonZeroIdx = 0;
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    if (nums[i] !== 0) {
      // Swap current element with the position where it should go
      // This automatically handles the case where i equals nonZeroIdx
